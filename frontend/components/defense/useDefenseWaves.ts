...
import {
  DEFENSE_BOSS_WAVE_CYCLE_MS,
  DEFENSE_BOSS_WAVE_NUMBER,
  DEFENSE_BOSS_WAVE_SPAWN_WINDOW_MS,
  DEFENSE_WAVE_BANNER_MS,
  DEFENSE_WAVE_CYCLE_MS,
  DEFENSE_WAVE_SPAWN_WINDOW_MS,
} from "./defenseWaveConstants";

function cycleMsForWave(wave: number): number {
  return wave === DEFENSE_BOSS_WAVE_NUMBER
    ? DEFENSE_BOSS_WAVE_CYCLE_MS
    : DEFENSE_WAVE_CYCLE_MS;
}

function spawnEndMsForWave(wave: number): number {
  return wave === DEFENSE_BOSS_WAVE_NUMBER
    ? DEFENSE_WAVE_BANNER_MS + DEFENSE_BOSS_WAVE_SPAWN_WINDOW_MS
    : DEFENSE_WAVE_BANNER_MS + DEFENSE_WAVE_SPAWN_WINDOW_MS;
}
...
export type DefenseWavesOptions = {
  ...
  /** 보스 웨이브 제한 시간(본편) 종료 시 호출 — 필드에 적이 남아 있으면 패배 */
  onBossWaveTimedDefeat?: () => void;
  /** 필드 적 수(타임아웃 판정용) */
...
export function useDefenseWaves(
  ...
  const [secondsUntilNextWave, setSecondsUntilNextWave] = useState(
    Math.ceil(cycleMsForWave(1) / 1000)
  );
  ...
  const fieldEnemyCountRef = useRef(fieldEnemyCount);
  fieldEnemyCountRef.current = fieldEnemyCount;

  const onBossWaveTimedDefeatRef = useRef(onBossWaveTimedDefeat);
  onBossWaveTimedDefeatRef.current = onBossWaveTimedDefeat;

  const bossWaveTimedDefeatFiredRef = useRef(false);
  ...
  useEffect(() => {
    ...
    bossWaveTimedDefeatFiredRef.current = false;
    setWaveNumber(1);
    setShowWaveStartBanner(true);
    setSpawnEnemiesEnabled(false);
    setSecondsUntilNextWave(Math.ceil(cycleMsForWave(1) / 1000));
  ...
  const applyWaveUiFromNow = useCallback((now: number) => {
    ...
    while (true) {
      const w = waveNumRef.current;
      const cycleHere = cycleMsForWave(w);
      if (base < cycleHere) break;

      if (w === DEFENSE_BOSS_WAVE_NUMBER) {
        if (
          onBossWaveTimedDefeatRef.current &&
          !bossWaveTimedDefeatFiredRef.current &&
          !completedAllWavesRef.current &&
          fieldEnemyCountRef.current > 0
        ) {
          bossWaveTimedDefeatFiredRef.current = true;
          onBossWaveTimedDefeatRef.current();
          return;
        }
      }
    ...
    const wLive = waveNumRef.current;
    const e = base;
    const cycleNow = cycleMsForWave(wLive);
    const spawnEnd = spawnEndMsForWave(wLive);

    setShowWaveStartBanner(e < BANNER);
    setSpawnEnemiesEnabled(e >= BANNER && e < spawnEnd);

    const remSim = cycleNow - e;
  ...
  const skipRestOfCurrentWave = useCallback(() => {
    ...
    const now = Date.now();
    waveBaseElapsedRef.current = cycleMsForWave(waveNumRef.current);
    lastTickRealRef.current = now;
