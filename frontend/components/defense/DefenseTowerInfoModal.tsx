...
function formatAttackIntervalSec(ms: number, mode: "single" | "aoe"): string {
  const s = ms / 1000;
...
function formatSlowDebuffLine(
  slowDurationMs: number,
  slowPercent: number
): string {
  const s = slowDurationMs / 1000;
  const t = Number.isInteger(s) ? s.toFixed(0) : s.toFixed(1);
  return `${t}초간 이동속도 ${slowPercent}% 감소`;
}
...
export default function DefenseTowerInfoModal({
  ...
  const isAoePulse = combat.attackStyle === "instant_aoe_all_in_range";
  return (
    <Modal
      visible={visible}
      transparent
        ...
        <View style={styles.card}>
          ...
          <View style={styles.rows}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>
                {formatAttackIntervalSec(
                  combat.attackIntervalMs,
                  isAoePulse ? "aoe" : "single"
                )}
              </Text>
            </View>
            {isAoePulse &&
            ...
            combat.slowPercent != null ? (
              <View style={styles.row}>
                <Text style={styles.label}>추가 효과</Text>
                <Text style={styles.value}>
                  {formatSlowDebuffLine(
                    combat.slowDurationMs,
                    combat.slowPercent
                  )}
                </Text>
              </View>
